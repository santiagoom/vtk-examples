...
void MainWindow::on_m_combineImage_clicked() {
    ...
    QStringList strPathList = QFileDialog::getOpenFileNames(this, tr("Choose input images"), tr("/home"),
                                                            tr("Images(*.mhd);;All files (*.*);;"));
    qDebug() << "+++++++0";
        ...
        for (int k = 0; k < depth; k++) {
            for (int j = 0; j < height; j++) {
                for (int i = 0; i < width; i++) {
                    ...
                    numPositive =
                            (pImage[index + 1] + pImage[index - 1] + pImage[index - width] + pImage[index + width] +
                             pImage[index - width * height] + pImage[index + width * height]) / 255.0f;
...
void MainWindow::on_m_extractParts_clicked() {
    ...
    Point3i point1 = Point3i(27, 37, 77);
...
//    points.append(point3);
    for (int i = 0; i < fileNames.size(); ++i) {
        ...
        QString outFileName = outDir + "/" + fileNames[i] + ".mhd";
        ESImageND *image = loadESImage(fileName);
        smoothImage(image);
//        writeESImage(filePath + "/SM" + fileNames[i]+".mhd",image);
...
void MainWindow::smoothImage(ESImageND *image) {
    float *pImage = reinterpret_cast<float *>(image->getDataPtr());
                    ...
                    for (int y = -1; y < 2; ++y) {
                        for (int z = -1; z < 2; ++z) {
                            ...
                            if (curX > -1 && curX < image->getWidth()
                                ...
                                && curZ > -1 && curZ < image->getDepth() && (abs(x) + abs(y) + abs(z) == 1)) {
                                int curIndex =
                                        curZ * image->getHeight() * image->getWidth() + curY * image->getWidth() + curX;
                                value += 1.0f / 6.0f * pImage[curIndex];
                            }
                        }
                    }
                }
...
//    }
    ...
    int breakCells = 0, breakCells2 = 0;
    for (int k = 0; k < image->getDepth(); ++k) {
        for (int j = 0; j < image->getHeight(); ++j) {
            for (int i = 0; i < image->getWidth(); ++i) {
                int index = k * image->getHeight() * image->getWidth() + j * image->getWidth() + i;
                ...
                if (pImage[index] > 0.5f) {
                    for (int x = -int(round(m_radius)); x < int(round(m_radius)) + 1; ++x) {
                        for (int y = -int(round(m_radius)); y < int(round(m_radius)) + 1; ++y) {
                            for (int z = -int(round(m_radius)); z < int(round(m_radius)) + 1; ++z) {
                                if (sqrt(x * x + y * y + z * z) <= m_radius)//default 2(3 is used for the first case)
                                {
                                    int curX = x + i;
                                    int curY = y + j;
                                    int curZ = z + k;
                                    if (curX > -1 && curX < image->getWidth()
                                        ...
                                        int curIndex = curZ * image->getHeight() * image->getWidth() +
                                                       curY * image->getWidth() + curX;
...
//                        }
//                    }
//                    float ratio1 = float(count1)/float(total1);
//                    if (ratio1 < 0.2)//default 0.3
//                    {
//                       pImage[index] = 0.0f;
////                       qDebug()<<"info:"<<count1<<total1;
//                       breakCells2 ++;
//                    }
//                }
            }
    ...
    qDebug() << "Break cells number: " << breakCells2;
}
...
void MainWindow::regionGrowth(ESImageND *image, QList<Point3i> points, QString fileName) {
    ...
    qDebug() << "Source Point: " << points[0].X() << points[0].Y() << points[0].Z()
             << pImage[sliceSize * points[0].Z() + width * points[0].Y() + points[0].X()];
    int num = 0;
    for (int i = 0; i < image->getImageSize(); ++i) {
        pOutputImage[i] = 0.0f;
        if (pImage[i] > 0.5f) {
    ...
    qDebug() << "Read positive value cells :" << num << "seeds:" << points.size();
            ...
            for (int j = -1; j < 2; ++j) {
                for (int i = -1; i < 2; ++i) {
                    ...
                    if (curX > -1 && curX < width
                        && curY > -1 && curY < height
                        && curZ > -1 && curZ < depth && (abs(k) + abs(j) + abs(i) == 1)) {
...
//                            qDebug()<<"Point : "<<Its<<pImage[index]<<pImage[seedIndex]<<x<<y<<z<<"results: "<<curX<<curY<<curZ;
                        }
                    }
                }
            }
...
void MainWindow::on_m_radius_editingFinished() {
    m_radius = ui->m_radius->text().toFloat();
    qDebug() << "m_radius: " << m_radius;
}

void MainWindow::on_m_ratio_editingFinished() {
    m_ratio = ui->m_ratio->text().toFloat();
